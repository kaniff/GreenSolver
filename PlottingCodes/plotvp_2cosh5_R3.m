...
plot(x,vp(:,k(1)),'LineWidth',3,'Color','b');
plot(x,vp(:,k(2)),'LineWidth',3,'Color','r');%,'MarkerSize',3,'MarkerEdgeColor','r','Marker','s');
plot(x,vp(:,k(3)),'LineWidth',3,'Color','k');
plot(x,vp(:,k(4)),'LineWidth',3,'Color',c(4,:));%,'LineStyle','-.');
...
% Create ylabel
ylabel('\it v_{p} \rm','FontSize',30,'FontName','Times');
...
filename=sprintf('Vp_2cosh5_R3_N23');
print(filename,'-dpdf')
